...
class ExtendedTrainer(Trainer):
    ...
    def train(self, output_dir, learning_rate, batch_size, weight_decay, epochs):
        logging.info(
        ...
        for epoch in range(epochs):
            ...
            torch.save(
                self.model.state_dict(), os.path.join(output_dir, f"model_{epoch}.pt")
            )
    ...
    def parallel_train(
        self, output_dir, learning_rate, batch_size, weight_decay, epochs
    ):
        logging.info(
            f"learning rate: {learning_rate}, batch size: {batch_size}, epochs: {epochs}"
        )
        ...
        train_dataloader, eval_dataloader, self.model, optimizer = accelerator.prepare(
            train_dataloader, test_dataloader, self.model, optimizer
        )
