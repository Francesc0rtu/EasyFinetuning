...
class ExtendedShapleyScorer(ShapleyScorer):
    def __init__(
        ...
        super(ExtendedShapleyScorer, self).__init__(
            input_data_path, output_data_path, checkpoint_path, subset_frac
        )
    ...
    def load_model(self, checkpoint_path: str):
        ...
        def f(x):
            tv = torch.tensor(
                [
                    tokenizer.encode(
                        v, padding="max_length", max_length=128, truncation=True
                    )
                    for v in x
                ]
            ).to(device)
